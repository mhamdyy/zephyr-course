...
ZTEST(ring_buf_push_pop, test_push_full_returns_enospc)
{
    ...
    zassert_equal(rb_push(99), -ENOSPC, "Buffer must be full");
    zassert_equal(rb_count(), 4, "Rejected push did not consume a slot");
}
...
ZTEST(ring_buf_boundaries, test_pop_null_returns_einval)
{
     ...
     */
    zassert_equal(rb_pop(NULL), -EINVAL, "pop(NULL) returns exactly -EINVAL");
}
